...
class MaterializeSourceClosureTest(unittest.TestCase):
    ...
    def test_required_license_notice_row_is_materialized(self) -> None:
        notice=self.repository/"upstream/COPYING"; notice.write_bytes(b"notice\n")
    ...
    def test_explicit_generated_directory_and_collisions(self) -> None:
        ...
        document["generated_directories"]=[{"path":"upstream","evidence":"fixture"}]; self.manifest.write_text(json.dumps(document)); self.assertIn("collides",self.invoke().stderr); self.assertFalse(self.destination.exists())
