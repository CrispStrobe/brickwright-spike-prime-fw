...
class ClosureTest(unittest.TestCase):
    ...
    def test_mbedtls_blank_cmake_selects_apache_from_pinned_license(self):
        import runpy
        module=runpy.run_path(str(TOOL)); source=self.base/"blank"; source.write_bytes(b"# blank\n"); boundary=self.base/"LICENSE"; boundary.write_bytes(b"choose Apache OR GPL; warranty\n")
        selection={"source_sha256":hashlib.sha256(source.read_bytes()).hexdigest(),"boundary_path":"LICENSE","boundary_sha256":hashlib.sha256(boundary.read_bytes()).hexdigest(),"declared":"Apache-2.0 OR GPL-2.0-or-later","concluded":"Apache-2.0","markers":[b"Apache OR GPL",b"warranty"]}
        self.assertEqual("Apache-2.0",module["validate_boundary_selection"](source,self.base,selection,"fixture")[0]); source.write_bytes(b"drift"); self.assertRaises(SystemExit,module["validate_boundary_selection"],source,self.base,selection,"fixture"); source.write_bytes(b"# blank\n"); outside=self.base.parent/"outside-license"; outside.write_bytes(boundary.read_bytes()); boundary.unlink(); boundary.symlink_to(outside); self.assertRaises(SystemExit,module["validate_boundary_selection"],source,self.base,selection,"fixture")
        production=module["REVIEWED_BOUNDARY_LICENSE_SELECTIONS"][("mbedtls","framework/CMakeLists.txt")]; self.assertEqual("11402351e38392230bb8934ba1095c0c0049a296c0f8821f76e4672dff54b490",production["boundary_sha256"])
        return
