...
class ClosureTest(unittest.TestCase):
    ...
    def test_only_exact_root_git_administration_is_excluded(self):
        ...
        trace=self.base/"git.trace"; trace.write_text(f'1 openat(AT_FDCWD, "{self.root}/.git", O_RDONLY) = 3\n1 openat(AT_FDCWD, "{github}", O_RDONLY) = 4\n1 openat(AT_FDCWD, "{gitignore}", O_RDONLY) = 5\n'); self.invoke("generate","--roots",self.roots,"--cwd",self.base,"--strace",trace,"--depfile",self.dep,"--output",self.manifest,"--sbom",self.sbom); data=json.loads(self.manifest.read_text()); self.assertEqual("directory",data["vcs_administration"][0]["kind"]); self.assertNotIn(str(self.base),json.dumps(data)); self.assertTrue({".github",".gitignore"}.issubset({x["path"] for x in data["files"]}))
    ...
    def test_capture_relocation_emits_canonical_link_evidence(self):
        ...
        linked=json.loads(evidence.read_text()); row=linked["objects"][0]; self.assertEqual("main.o",row["path"]); self.assertEqual(["nuttx/main.c"],row["sources"]); self.assertTrue(row["mentioned_in_map"])
        emitted=self.manifest.read_text()+evidence.read_text(); self.assertNotIn(str(old),emitted); self.assertNotIn(str(self.base),emitted)
