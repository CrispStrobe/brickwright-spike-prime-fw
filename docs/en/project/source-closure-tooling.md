...
after checking their hashes. Generated rows default to `materialize: true`.
An exact row may set `materialize: false` only for an ephemeral,
same-generation product retained in the manifest, SPDX document, and link
evidence but regenerated by the offline build; its destination remains
reserved while stale worktree bytes are neither required nor copied.
Empty build topology is explicit: generated-input declarations may list
evidenced repository-relative `directories`. Generation verifies each existing
non-symlink directory, and materialization recreates only those manifest rows;
paths, collisions, and symlink traversal remain fail-closed.
...
Exact source-root `.git` administrative files, or descendants beneath a real
root `.git` directory, are excluded and recorded only by root, kind, count,
and reason; their contents and host paths are
...
The exact Twm4Nx top-level and `apps/Kconfig` files and NxWM `Kconfig` are also
