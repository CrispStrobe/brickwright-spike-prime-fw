...
- `btsensor_cmd.c` — Commit D 新規。`RFCOMM_DATA_PACKET` で受信した PC からのバイト列を 64B 行バッファに溜め、`'\n'` で `process_line()` を呼んで `strtok_r` ディスパッチ。応答 (`OK\n` / `OK <idx>\n` / `ERR <reason>\n`) は `btsensor_tx_enqueue_response()` で送る。コマンドは `IMU ON/OFF`、`SENSOR ON/OFF/MODE/SEND/PWM`、`SET ODR/ACCEL_FSR/GYRO_FSR <val>`、Issue #139 で追加された `GET ODR/ACCEL_FSR/GYRO_FSR` (driver enum idx を返す)。
- `imu_sampler.c` — uORB `/dev/uorb/sensor_imu0` の fd を btstack data source として登録し、各 `struct sensor_imu` (24 B: accel/gyro 生 int16 + ISR timestamp + per-sample `odr_idx`/`fsr_xl_idx`/`fsr_gy_idx`) をワイヤフォーマットのサンプルスロットへコピー、Kconfig 既定 8 サンプル (1〜80 範囲) を 1 フレームにまとめて `btsensor_tx_try_enqueue_frame()` 経由で送信。Commit D で `IMU ON/OFF` トグルと SET API (`set_odr_hz` / `set_accel_fsr` / `set_gyro_fsr`) を追加。Issue #139 で **local cache を撤去** し、SET は driver ioctl を直接呼ぶ thin wrapper、GET (`get_odr_idx` / `get_accel_fsr_idx` / `get_gyro_fsr_idx`) は driver の GET ioctl 経由で **live 値の enum idx** を取得する。`g_enabled` ガードは撤去され streaming 中の SET も driver に直接届く。
...
nsh> btsensor unpair                      # 保存済 Bluetooth ペアリングを全削除 (Issue #72)
...
- `btsensor bt on` と短押しは **link-key-aware** (Issue #71): btstack の link-key DB に既にペアリング済 peer があれば OFF/FAIL_BLINK → `CONNECTABLE` (silent 再接続、inquiry 非露出)、DB が空なら OFF/FAIL_BLINK → `ADVERTISING` (新規 pair 受付)。CONNECTABLE/ADVERTISING/PAIRED は no-op
- 長押し: OFF/FAIL_BLINK → **強制 ADVERTISING** (link-key DB を無視するエスケープハッチ。既存ペアリングが居ても新規 PC を追加する時に使う)、ADVERTISING/CONNECTABLE → OFF、PAIRED → RFCOMM 切断 + OFF
- `btsensor bt off` は無条件 (どの reachable 状態からでも OFF へ)
- `btsensor unpair` (Issue #72) は btstack の link-key DB を空にする (BT 状態は触らない)。次の `bt off → bt on` では DB 空 ⇒ link-key-aware ロジックが BT_ADVERTISING に振るので、リブート無しで旧 PC を切って別 PC と再 pair できる
- LED (Issue #73、状態ごとに rhythm を変えて区別、色は全て青):
    - `OFF` — 消灯
    - `ADVERTISING` — 50% duty の対称点滅、周期 = `CONFIG_APP_BTSENSOR_LED_BLINK_PERIOD_MS` (default 1 s)
    - `CONNECTABLE` — ダブルブリンク (短 ON ~100ms × 2 + 長 OFF rest)。「ti-tick . . . . . . ti-tick」のリズムで ADVERTISING の symmetric blink と一目で区別できる
    - `PAIRED` — 青常時点灯
    - `FAIL_BLINK` — `CONFIG_APP_BTSENSOR_LED_FAIL_BLINKS` 回 (~150ms × 2N) の青パルスを 1 ショット、その後消灯
...
| コマンド | 動作 | 制約 |
|---|---|---|
| `IMU ON\n` | btsensor 側のサンプリング開始 (driver fd を open) | — |
| `IMU OFF\n` | btsensor 側のサンプリング停止 (btsensor が握る fd を close)。driver 自身は drivebase 等他の subscriber が握っている限り動作継続 | — |
| `SET ODR <hz>\n` | ODR を変更 (13/26/52/104/208/416/833/1660/3330/6660 Hz) | live SET 可 (streaming 中も可)。Issue #139 で IMU OFF 制約撤去 |
| `SET ACCEL_FSR <g>\n` | 加速度 FSR (2/4/8/16) | live SET 可 |
| `SET GYRO_FSR <dps>\n` | ジャイロ FSR (125/250/500/1000/2000) | live SET 可 |
| `GET ODR\n` | 現在 ODR を **driver enum idx** で取得 | reply `OK <idx>\n` |
| `GET ACCEL_FSR\n` | 現在加速度 FSR を **driver enum idx** で取得 | reply `OK <idx>\n` |
| `GET GYRO_FSR\n` | 現在ジャイロ FSR を **driver enum idx** で取得 | reply `OK <idx>\n` |

応答パターン:
- SET 成功: `OK\n`
- GET 成功: `OK <idx>\n` (idx は enum 値)
- ioctl 失敗: `ERR errno=<n>\n`
- 不正な値 / トークン: `ERR invalid <token>\n`
- 行が 64B 超: `ERR overflow\n`
- 未知のコマンド: `ERR unknown <cmd>\n`

実装メモ (Issue #139 反映後):
- SET / GET は **transient な O_WRONLY fd** を一時的に open して ioctl を発行する (`imu_open_ctrl_fd()`)。`O_WRONLY` はセンサ upper-half の自動 activate (O_RDOK でのみ発火) を回避するので、btsensor は subscriber count を増やさずに driver を制御できる
- driver 側 (`lsm6dsl_uorb.c`) の active 中 `-EBUSY` ガードは Issue #139 で撤去。SET ハンドラは `devlock` 配下で `push_data()` と直列化されるため、register R-M-W と publish path のレースは発生しない。SET 直後 1 サンプルの過渡 (idx 切替境界) は consumer 側の recalibrate で吸収
- **local cache は撤去** された (Issue #139)。GET は毎回 driver の GET ioctl を呼んで live 値を取得。`struct sensor_imu` に埋め込まれた per-sample idx と GET ioctl の戻り値は同一表現 (`lsm6dsl_odr_e` / `lsm6dsl_fsr_xl_e` / `lsm6dsl_fsr_gy_e`)
- bundle_emitter は drain した sample 群の idx 不一致を検出すると **frame を分割して emit** する。BUNDLE header の rate/fsr は samples[0] の idx を `g_odr_hz_table[]` 等の lookup で物理値に変換して書く。0-sample frame は last-known idx cache で fallback
- driver 側 `cfg_odr` はユーザ設定の ODR を保持し、`activate(true)` 時に live HW 書き込みする。SET ioctl が active 中に呼ばれた場合は cfg_odr 更新と live HW 書き込みを両方実施し、HW 書き込み失敗時は cfg_odr を旧値に rollback
