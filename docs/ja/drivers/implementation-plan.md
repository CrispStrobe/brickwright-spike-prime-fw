...
| # | ドライバ | デバイスパス | 優先度 | 依存 |
|---|---|---|---|---|
| 1 | I/O ポート検出 (DCM) | `/dev/legoport[0-5]` | **済** | GPIO、Issue #42 |
| 2 | LUMP UART プロトコル | (内部) + ioctl | **済** | UART4/5/7/8/9/10、Issue #43、[lump-protocol.md](lump-protocol.md) |
| 3 | H-Bridge モーター制御 | `/dev/legomotor[N]` | **P0** | TIM1/3/4 PWM、Issue #44 |
| 4 | センサーデータ読取り | `/dev/uorb/sensor_lego[N]` | **済** | LUMP、Issue #45、[sensor.md](sensor.md) |
...
| 13 | Bluetooth (CC2564C) | `/dev/bnep0` | **済** | USART2 + DMA1 S6/S7、[bluetooth.md](bluetooth.md) |
...
#### 1c. LUMP UART プロトコル (済、Issue #43)

- LEGO UART Messaging Protocol の実装。詳細は [lump-protocol.md](lump-protocol.md)
- 同期フェーズ: 115200 baud で SPEED probe → ACK 来なければ 2400 fallback → CMD_TYPE → INFO frames → SYS_ACK → 10ms wait → negotiated baud (典型 115200)
- データフェーズ: 100 ms `SYS_NACK` keepalive + DATA frame 受信 (~100 Hz/port)
- 6 ポート分の kthread を boot 時 pre-create、DCM ハンドオフ CB が `nxsem_post()` で wake
- per-port watchdog (1 ms × 2000 = 2 s) + capped exp backoff (100ms→1s→5s→30s) で disconnect/復旧
...
#### 2a. センサーデータ読取り (済、Issue #45)

- LUMP データフェーズで受信したセンサーデータを **NuttX uORB sensor framework** 経由で `/dev/uorb/sensor_lego[N]` (N=0..5) で公開。詳細は [sensor.md](sensor.md)
- 固定 56 byte `struct lump_sample_s` envelope + C union (`raw[32]/i8/i16/i32/f32`) で動的 mode + 動的 data shape を吸収。`data_type` で switch して直アクセス
- mode 切替・writable mode TX は custom ioctl (`LEGOSENSOR_SELECT/SEND/CLAIM/RELEASE`)、CLAIM/RELEASE で制御権を arbitrate。auto-release は `sensor_ops_s::close(filep)` 経由
- 対応: カラーセンサー (Type 61)、超音波センサー (Type 62)、フォースセンサー (Type 63)、および BOOST/Technic/SPIKE motor encoder telemetry (Type 38/46/47/48/49/65/75/76)
...
#### 4a. Bluetooth (CC2564C) — **実装済** (Issue #47)

- USART2 (PD5/PD6) + フロー制御 (PD3/PD4)、DMA1 Stream 6 (TX Ch4) / Stream 7 (RX Ch6)
...
Phase 3b: MP2639A 充電      (済 - /dev/charge0, BCD on LPWORK)
    |
Phase 4a: Bluetooth         (済 - /dev/bnep0, HCI 3 Mbps)
```
