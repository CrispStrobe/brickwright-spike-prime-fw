...
- **heading 軸 kV は Step 6.5 で一旦 0 据え置き → Phase 7 で覆した (§8.5.7 / §8.5.8)**: Step 6.5 で turn 90 brake を 3 構成で比較:
  ...
  - Step 6.5 時点では「kV=6 が turn 90 を regression」と読めたため heading FF を 0 に据えた。Phase 7 (§8.5.7) でこれが **move-length 交絡**と判明: kV=6 は長 move (turn 180, #156) を直し、turn 90 の regression は短 move の静止摩擦 trap (`straight 50`, #155 と共通) であって heading-FF-model の欠陥ではない。**production は現在 `ff_head_kV=6` を terminal breakaway floor (§8.5.8) とセットで出荷**。
- **kA は両軸とも 0 維持** — 下記の decel-phase 過剰減速は実在かつ軸非依存。覆ったのは heading 軸 `kV` のみ。
...
cfg key (Step 6.1; "default" は Phase 6.6 / Phase 7 retune 後の現行 compiled default — これらの key は 0 で導入された):

| key | default | 範囲 | 意味 |
|---|---|---|---|
| `ff_dist_kV` | 9 | [-1000, +1000] | distance 軸の速度 FF (.01% per deg/s) |
| `ff_dist_kA` | 0 | [-1000, +1000] | distance 軸の加速度 FF (.01% per deg/s²) |
| `ff_head_kV` | 6 | [-1000, +1000] | heading 軸の速度 FF (Phase 7, §8.5.8) |
...
- `ff_head_kV=6` は正しい plant 値 (SysId = distance と同一) で turn 180 を直すが、単独採用は turn 90 を壊す → §8.5.8 で terminal breakaway とセット採用して解消
- follow-up Phase 7 本体 = `ff_head_kV=6` 採用 **+ 短 move stiction 対策** (#155 と共通) をセットで実施 → これにより #156 も解消 (§8.5.8)
- §8.5.6 の decel-decouple 案は kV=6 が overshoot を直したため優先度低下

#### 8.5.8 Phase 7 本体: 終端 static-friction breakaway (Issue [#158](https://github.com/owhinata/spike-nx/issues/158))

§8.5.7 の follow-up を実施: `ff_head_kV=6` を採用し、**per-motor の終端 static-friction breakaway floor** とセットにして、`straight 50` (#155) と `turn 90` (kV=6 単独なら起きる regression) が共有する短 move trap を解消した。

**機構** (`apply_breakaway_floor`, `drivebase_drivebase.c`, L/R compose 内):

- kS (§8.5.2) は走行中の Coulomb 補償で、`v_ref→0` で sign が 0 に落ち、かつ上限 10% なので、停止した輪を breakaway させられない (静止摩擦 ≈ 25%)。
- 有限 move の trajectory が done でも輪が 3000-mdeg deadband の外にいるとき、その輪の pre-clamp duty を **per-wheel 位置誤差の方向**へ最低 `ff_terminal_breakaway` (.01% 単位) まで底上げする (`D∓H` compose・int64 — kS と同じ per-motor 正しさ、Plan D1)。
- **saturating (加算でなく floor)**: 既に正方向へ floor 以上を出している輪は不変なので cruise/通常整定を侵さず、duty rail へ積み上がらない。
- **per-move one-shot**: episode は floor を初適用した時点で開始し、輪が deadband 到達 or target 通過 (誤差符号反転) で consume されるので、慣性による 1 回の通過は PID に任せる — 逆方向の再点火 (buzz) なし。逆符号の指令 (`v_ref=0` でも speed 項で接近中の輪を PID が制動している) は尊重し上書きしない。
- 新 cfg key `ff_terminal_breakaway` (範囲 `[0, 3000]`; 30% 上限は静止摩擦を超えるため FF gain 上限 10% を意図的に超過)。compiled default は **2600**、0 で機構無効。RT-path の 64-bit divide は不変。

**Bench sweep** (`ff_head_kV=6`, brake, 各 2 run, vbat ≈ 8.57-8.59 V):
...
2000 は breakaway 閾値未満 (新品電池では battery latch が適用 duty を ≈×0.84 に縮めるため 2000 → 実効 ~17% で、実測 breakaway ~18-20% を下回る) で turn 90 が不安定。2400 と 2600 は同等かつ安定で、**2600 を compiled default** とした (モータ摩擦/電池 drift への余裕)。これで #155 と #156 を同時に解消する。
