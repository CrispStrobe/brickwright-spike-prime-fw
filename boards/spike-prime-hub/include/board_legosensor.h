...
#define LEGOSENSOR_GET_LATEST        _SNIOC(0x00f8)  /* lump_sample_s *             */
...
/* `LEGOSENSOR_GET_LATEST` copies the newest DATA envelope for the class
 * without advancing any uORB subscriber cursor.  It returns `-ENODATA`
 * until a DATA frame has arrived after the current bind, `-ENODEV` while
 * unbound, and `-EAGAIN` if ownership changes during the snapshot.  SYNC and
 * disconnect sentinels are deliberately not returned.
 */
