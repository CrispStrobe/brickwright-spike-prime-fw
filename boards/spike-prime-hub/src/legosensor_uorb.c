...
struct legosensor_port_s
{
  ...
  bool     latest_valid;               /* newest DATA envelope available */
  struct lump_sample_s latest;         /* non-destructive ioctl snapshot */
  ...
  switch (cmd)
    {
      ...
      case LEGOSENSOR_GET_LATEST:
        {
          FAR struct lump_sample_s *out =
              (FAR struct lump_sample_s *)(uintptr_t)arg;
          struct lump_sample_s sample;
          ...
          /* Capture the binding identity, copy under the port lock, then
           * verify the identity once more.  The driver never nests bind and
           * port locks, so this retry-style validation preserves the global
           * lock order and never consumes the uORB ring.
           */

          nxmutex_lock(&g_bind_lock);
          bound_port = cs->bound_port;
          bind_generation = cs->bind_generation;
          nxmutex_unlock(&g_bind_lock);
          ...
          p = &g_legosensor_port[bound_port];
          nxmutex_lock(&p->lock);
          valid = p->latest_valid;
          if (valid)
            {
              sample = p->latest;
            }
          ...
          nxmutex_lock(&g_bind_lock);
          if (cs->bound_port != bound_port ||
              cs->bind_generation != bind_generation)
            {
              ret = -EAGAIN;
            }
          else if (!valid)
