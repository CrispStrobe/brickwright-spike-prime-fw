 ...
 * with the telemetry scheduler, and caches the latest publish per class.
...
static int class_open(struct class_state_s *cls)
{
  ...
  cls->fd = fd;
  int rc = btsensor_scheduler_watch_start(&cls->watch, fd, on_read, cls);
  if (rc != 0)
    {
      close(fd);
      cls->fd = -1;
...
static void class_close(struct class_state_s *cls)
{
  ...
  btsensor_scheduler_watch_stop(&cls->watch);
  close(cls->fd);
  cls->fd = -1;
...
int sensor_sampler_set_pwm_with_port(uint8_t class_id,
                                     const int16_t *channels, size_t n,
                                     uint8_t *port_id)
{
  if (class_id >= BTSENSOR_TLV_COUNT || channels == NULL || port_id == NULL)
    {
      return -EINVAL;
    }
  ...
  int rc = ioctl(fd, LEGOSENSOR_GET_INFO, (unsigned long)&info);
  if (rc < 0)
    {
      int err = -errno;
      close(fd);
  ...
  if (info.port >= BOARD_LEGOPORT_COUNT)
    {
      close(fd);
      return -ERANGE;
    }
  ...
  rc = ioctl(fd, LEGOSENSOR_SET_PWM, (unsigned long)&pwm);
  if (rc < 0)
    {
      int err = -errno;
      close(fd);
...
{
  uint8_t port_id;
  return sensor_sampler_set_pwm_with_port(class_id, channels, n, &port_id);
}
