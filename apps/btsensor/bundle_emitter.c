...
static struct btsensor_timer_s g_tick_timer;
...
static int timer_arm(void)
{
  ...
  int rc = btsensor_scheduler_timer_start(
      &g_tick_timer, CONFIG_APP_BTSENSOR_BUNDLE_TICK_MS,
      tick_handler, NULL);
  if (rc != 0)
    {
      return rc;
...
static void timer_disarm(void)
{
  ...
  btsensor_scheduler_timer_stop(&g_tick_timer);
  g_timer_armed = false;
}
...
static void tick_handler(void *arg)
{
   ...
   * is event-driven via scheduler fd watches, so its cache stays fresh
   * on its own.
   */
  ...
  /* The neutral scheduler keeps periodic timers armed. */
      ...
      if (rc != 0)
        {
          ...
          g_imu_on = false;
          pthread_mutex_unlock(&g_state_lock);
          (void)imu_sampler_set_enabled(false);
          return rc;
        }
    }
  else if (!sensor_on)
      ...
      if (rc != 0)
        {
          ...
          g_sensor_on = false;
          pthread_mutex_unlock(&g_state_lock);
          (void)sensor_sampler_set_enabled(false);
          return rc;
        }
    }
  else if (!imu_on)
