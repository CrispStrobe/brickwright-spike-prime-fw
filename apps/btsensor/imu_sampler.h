 ...
 * pulls every available sample into a private ring on each scheduler fd
 * callback, and exposes a non-blocking drain API for
 ...
 * driver is dropped to OFF.  Waits for any active scheduler callback.
 */
 ...
 * first), returning the count.  Called by the bundle emitter tick.
 * Samples that don't fit in this drain remain
