 ...
 * `set_enabled(true)`, registers each fd with the telemetry scheduler, and
 ...
 * is released.  Waits for active scheduler callbacks before returning.
 */

void sensor_sampler_deinit(void);

/* Toggle SENSOR streaming.  on=true opens the 6 /dev/uorb/sensor_*
 * topics and registers them with the telemetry scheduler; on=false closes
 * them.  No `LEGOSENSOR_CLAIM`
 ...
 *
 * Thread-safe with scheduler updates.
 */
...
/* As set_pwm(), additionally returning the bound physical port.  Motor
 * callers that implement transport disconnect failsafes must use this form;
 * PWM is not issued unless the port identity can be established. */
